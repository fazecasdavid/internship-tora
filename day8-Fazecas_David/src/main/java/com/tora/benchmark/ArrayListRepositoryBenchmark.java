...
import com.tora.reposiotry.impl.ArrayListBasedRepository;
...
public class ArrayListRepositoryBenchmark {
    ...
    public void testAdd(ArrayListRepositoryState state) {
    ...
    @Warmup(iterations = 5, time = 5, timeUnit = TimeUnit.MILLISECONDS)
    @Measurement(iterations = 5, time = 100, timeUnit = TimeUnit.MILLISECONDS)
    public void testContains(ArrayListRepositoryState state) {
    ...
    public void testRemove(ArrayListRepositoryState state) {
        int nextId = state.NO_EPOCHS - state.currentId;
        if(state.currentId % 2 == 0){
            nextId *= 2;
        }
