...
import com.tora.reposiotry.impl.HashSetBasedRepository;
...
public class HashSetRepositoryBenchmark {
    ...
    public void testAdd(HashSetRepositoryState state) {
    ...
    @Warmup(iterations = 5, time = 5, timeUnit = TimeUnit.MILLISECONDS)
    @Measurement(iterations = 100, time = 10, timeUnit = TimeUnit.MILLISECONDS)
    public void testContains(HashSetRepositoryState state) {
    ...
    @Warmup(iterations = 5, time = 5, timeUnit = TimeUnit.MILLISECONDS)
    @Measurement(iterations = 100, time = 10, timeUnit = TimeUnit.MILLISECONDS)
    public void testRemove(HashSetRepositoryState state) {
        int nextId = state.NO_EPOCHS - state.currentId;
        if(state.currentId % 2 == 0){
            nextId *= 2;
        }
    ...
    public static class HashSetRepositoryState {
