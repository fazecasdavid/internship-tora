...
import com.tora.reposiotry.impl.fastutil.FastutilSetBasedRepository;
...
public class FastutilSetRepositoryBenchmark {
    @Benchmark
    ...
    public void testAdd(FastutilSettRepositoryState state) {
    ...
    @Warmup(iterations = 5, time = 5, timeUnit = TimeUnit.MILLISECONDS)
    @Measurement(iterations = 100, time = 10, timeUnit = TimeUnit.MILLISECONDS)
    public void testContains(FastutilSettRepositoryState state) {
    ...
    @Warmup(iterations = 5, time = 5, timeUnit = TimeUnit.MILLISECONDS)
    @Measurement(iterations = 100, time = 10, timeUnit = TimeUnit.MILLISECONDS)
    public void testRemove(FastutilSettRepositoryState state) {
        int nextId = state.NO_EPOCHS - state.currentId;
        if(state.currentId % 2 == 0){
            nextId *= 2;
        }
    ...
    public static class FastutilSettRepositoryState {
        ...
        public void doSetup() {
            ...
            repository = new FastutilSetBasedRepository<>();
            currentId = 0;
