...
import com.tora.reposiotry.impl.TreeSetBasedRepository;
...
public class TreeSetRepositoryBenchmark {
    @Benchmark
    ...
    public void testAdd(TreeSetRepositoryState state) {
    ...
    @Warmup(iterations = 5, time = 5, timeUnit = TimeUnit.MILLISECONDS)
    @Measurement(iterations = 100, time = 10, timeUnit = TimeUnit.MILLISECONDS)
    public void testContains(TreeSetRepositoryState state) {
    ...
    @Warmup(iterations = 5, time = 5, timeUnit = TimeUnit.MILLISECONDS)
    @Measurement(iterations = 100, time = 10, timeUnit = TimeUnit.MILLISECONDS)
    public void testRemove(TreeSetRepositoryState state) {
        int nextId = state.NO_EPOCHS - state.currentId;
        if(state.currentId % 2 == 0){
            nextId *= 2;
        }
